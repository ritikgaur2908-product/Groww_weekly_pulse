...
We are building an automated weekly “pulse” that turns public App Store and Google Play reviews for selected fintech products into a one-page insight report and delivers it to stakeholders through Google Workspace, using MCP (Model Context Protocol). We will create an MCP server separately in the later stages of the project and provide it in this project so that writes to Google Docs and Gmail go through this dedicated local MCP server—not ad hoc API calls inside the agent.
...
* Ingest public reviews from the last 8–12 weeks (configurable window) from Google Play (scraper-based).
...
* Deliver outputs only through our custom Google Workspace MCP server:
...
| Human-visible delivery| Custom MCP server provided in this project (Google Docs + Gmail integration) |

The agent is an MCP host/client; it does not embed Google credentials or call the Docs/Gmail REST APIs directly for delivery. These will be handled by the separately created MCP server included in the project.

## Key requirements
* **MCP-based delivery**: Append to the shared Google Doc and send Gmail only via the custom project-provided MCP server's tools (e.g. document batch update, draft/create/send flows as defined in architecture).
...
* Storing Google OAuth secrets in the agent codebase—they belong in the custom MCP server’s configuration, per architecture.
